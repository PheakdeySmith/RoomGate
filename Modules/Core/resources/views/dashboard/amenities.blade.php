...
<div class="container-xxl flex-grow-1 container-p-y">
  @if (!($canCreateAmenity ?? true))
    <div class="alert alert-warning d-flex align-items-start" role="alert">
      <i class="icon-base ti tabler-alert-triangle me-2"></i>
      <div>
        <div class="fw-semibold">Plan limit reached</div>
        <div>You have reached your amenity limit. Upgrade your plan to add more amenities.</div>
      </div>
    </div>
  @endif
  <div class="card">
              ...
              <td>
                <div class="d-flex align-items-center">
                  <a href="javascript:;" class="btn btn-icon btn-text-secondary rounded-pill waves-effect me-1" data-bs-toggle="modal" data-bs-target="#editAmenityModal"
                    ...
                    data-amenity-name="{{ $amenity->name }}"                    data-amenity-price="{{ number_format(($amenity->price_cents ?? 0) / 100, 2, '.', '') }}"
                  ...
                  <form method="POST" action="{{ route('core.amenities.destroy', $amenity) }}" data-confirm="Delete this amenity?">
                    @csrf
                    @method('DELETE')
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{ route('core.amenities.store') }}" class="row g-3">
          @csrf
          <div class="col-md-6">
...
@push('page-scripts')
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      const canCreateAmenity = @json($canCreateAmenity ?? true);
      const amenityLimit = @json($amenityLimit ?? null);
      const addAmenityText = canCreateAmenity
        ? '<i class="icon-base ti tabler-plus me-0 me-sm-1 icon-16px"></i><span class="d-none d-sm-inline-block">Add Amenity</span>'
        : '<span class="d-none d-sm-inline-block">Limit reached</span>';

      initTable(
        '.datatables-amenities',
        'Search Amenity',
        addAmenityText,
