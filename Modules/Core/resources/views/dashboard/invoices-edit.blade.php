...
@section('title', 'Edit Invoice | RoomGate')
...
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="row invoice-add">
    <div class="col-lg-9 col-12 mb-lg-0 mb-6">
      <form method="POST" action="{{ route('core.invoices.update', ['tenant' => $tenant->slug, 'invoice' => $invoice]) }}" class="card invoice-preview-card p-sm-12 p-6" id="invoiceForm">
        @csrf
        @method('PATCH')
          ...
          <div class="d-flex flex-wrap flex-column flex-sm-row justify-content-between text-heading">
            ...
            <div class="col-md-5 col-8 pe-0 ps-0 ps-md-2">
              <dl class="row mb-0 gx-4">
                ...
                <dd class="col-sm-7">
                  <input type="text" name="issue_date" class="form-control flatpickr" placeholder="YYYY-MM-DD" value="{{ optional($invoice->issue_date)->format('Y-m-d') }}" required />
                </dd>
                <dt class="col-sm-5 d-md-flex align-items-center justify-content-end">
                  <span class="fw-normal">Due Date:</span>
                </dt>
                <dd class="col-sm-7 mb-0">
                  <input type="text" name="due_date" class="form-control flatpickr" placeholder="YYYY-MM-DD" value="{{ optional($invoice->due_date)->format('Y-m-d') }}" required />
                </dd>
              </dl>
            </div>
          </div>
                ...
                <select class="form-select" id="contractSelect" name="contract_id" required>
                  ...
                  @foreach ($contracts as $contract)
                    ...
                    <option
                      ...
                      @selected($contract->id === $invoice->contract_id)>
        ...
        <div class="card-body px-0">
          <div class="row row-gap-4">
            <div class="col-md-6 mb-md-0 mb-4">
              <input type="text" class="form-control" name="notes" id="invoiceMsg" placeholder="Invoice note (optional)" value="{{ $invoice->notes }}" />
            </div>
    ...
    <div class="col-lg-3 col-12 invoice-actions">
      <div class="card mb-6">
        <div class="card-body">
          <a href="{{ route('Core.invoices.preview', ['tenant' => $tenant->slug, 'invoice' => $invoice]) }}" class="btn btn-label-secondary d-grid w-100 mb-4">Preview</a>
          <button type="submit" form="invoiceForm" class="btn btn-primary d-grid w-100">Update Invoice</button>
        </div>
      </div>
      <div>
        ...
        <select class="form-select mb-6" id="statusSelect" name="status" form="invoiceForm">
          ...
          <option value="partial" @selected($invoice->status === 'partial')>Partial</option>
...
@push('page-scripts')
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      let utilityBills = [];
      let manualItems = @json($manualItems);
      const selectedUtilityIds = @json($selectedUtilityIds);
        ...
        fetch(`{{ route('core.invoices.utilities') }}?contract_id=${contractId}`)
          ...
          .then((payload) => {
            utilityBills = (payload.data || []).map((bill) => ({
              ...bill,
              selected: selectedUtilityIds.includes(bill.id)
            }));
